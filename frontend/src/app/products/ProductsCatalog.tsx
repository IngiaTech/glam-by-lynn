/**
 * Products catalog — the interactive half of /products.
 *
 * The first page of products, the categories and the `?category=` deep link are
 * resolved by the server component in `page.tsx` and passed in as props, so the
 * listing is present in the initial HTML. This component owns the filtering,
 * search, pagination, cart and wishlist behaviour from there on.
 *
 * Note it deliberately does *not* call `useSearchParams`: that forces the
 * subtree into a Suspense boundary, and static prerendering then emits the
 * fallback spinner instead of the catalogue — which was exactly the H11 bug.
 */
...
interface ProductsCatalogProps {
  ...
  /** `?category=<slug>` as read by the server component. */
  initialCategorySlug?: string;
}

function deriveBrands(products: Product[]): Brand[] {
  const unique = new Map<string, Brand>();
  products.forEach((product) => {
    if (product.brand && !unique.has(product.brand.id)) {
      unique.set(product.brand.id, product.brand);
    }
  });
...
export default function ProductsCatalog({
  ...
  // State — seeded from the server render so the first paint has real content.
  const [products, setProducts] = useState<Product[]>(initialProducts);
  const [brands, setBrands] = useState<Brand[]>(() => deriveBrands(initialProducts));
  ...
  // Filter state. The `?category=` deep link is resolved here, on the first
  // render, rather than by a post-hydration effect — so the server and client
  // agree about which filter is active and the listing never flashes.
  const [filters, setFilters] = useState<ProductFilters>(() => {
    const match = initialCategorySlug
      ? initialCategories.find((category) => category.slug === initialCategorySlug)
      : undefined;
  ...
  // The server already fetched the first page with these exact filters, so skip
  // the initial run and refetch only once the user actually changes something.
  ...
  // Categories arrive from the server render. Only fetch here if that came back
  // empty (an API hiccup during SSR), so the filter Select can still populate.
  useEffect(() => {
